	...
	private function getList()
	{
       ...
       foreach ($results as $result) {
		...
		$this->data['filter_name'] = $filter_name;
			...
			if (SC_VERSION < 22 || SC_VERSION > 22) {
				$this->url = new Url($this->config->get('config_secure') ? HTTPS_CATALOG : HTTP_CATALOG);
			} else {
       ...
       */
		...
		$this->config->set('config_url', HTTP_SERVER);
			...
			if (SC_VERSION < 22 || SC_VERSION > 22) {
				$this->url = new Url($this->config->get('config_secure') ? HTTPS_SERVER : HTTP_SERVER);
			} else {
				$this->url = new Url($this->config->get('config_secure'));
			}
