...
class ModelRecordTreeComments extends Model
{
	public function getCommentsByMarkId($data = Array(), $mark = 'product_id', $settings_widget = array())
	{
		if (!empty($data)) {
			...
			if (!isset($cdata[$mark_id])) {
				...
				if (isset($this->request->get['product_id']) && $mark == 'product_id') {
					$sql_product        = "LEFT JOIN " . DB_PREFIX . "order_product op ON (op.order_id = o.order_id AND o.customer_id <> 0 AND o.order_status_id IN " . $this->db->escape($sql_status) . " AND op.product_id = '" . (int) $this->request->get['product_id'] . "')";
        ...
        LEFT JOIN `" . DB_PREFIX . "order` o ON ( r.customer_id = o.customer_id AND o.customer_id <> 0 AND o.order_status_id IN " . $this->db->escape($sql_status) . " )
        " . $sql_product . "
		" . $sql_rf . "
		WHERE p." . $this->db->escape($mark) . " = '" . (int) $mark_id . "'
		AND p.status = '1'
		AND r.status = '1'
