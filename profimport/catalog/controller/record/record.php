...
class ControllerRecordRecord extends Controller
{
	...
	public function index()
	{
		...
		if (isset($this->data['settings_general']['reviews_widget_status']) && $this->data['settings_general']['reviews_widget_status']) {
			...
			if (SC_VERSION > 15 && count($results) > 0 && isset($this->data['settings_general']['related_theme']) && $this->data['settings_general']['related_theme']) {
				...
				agoo_cont($str_controller_featured_path, $this->registry);

				if (isset($this->data['settings_general']['related_theme_title'][$this->config->get('config_language_id')])) {
					$this->load->setreplacedata(array($str_controller_featured_path.'.tpl' => array('heading_title'=> $this->data['settings_general']['related_theme_title'][$this->config->get('config_language_id')])));
				}
