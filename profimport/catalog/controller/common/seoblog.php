...
if (!class_exists('ControllerCommonSeoBlog')) {
	class ControllerCommonSeoBlog extends Controller
	{
		...
		public function __construct($registry)
		{
			...
			if ((isset($this->data['settings_general']['seocms_url_secure']) && $this->data['settings_general']['seocms_url_secure'] == 'https' && $this->data['settings_general']['seocms_url_secure'] != 'http') || ((isset($_SERVER['HTTPS']) && ($_SERVER['HTTPS'] == 'on' || $_SERVER['HTTPS'] == '1')) || (!empty($_SERVER['HTTP_X_FORWARDED_PROTO']) && (strtolower($_SERVER['HTTP_X_FORWARDED_PROTO']) == 'https') || (!empty($_SERVER['HTTP_X_FORWARDED_SSL']) && $_SERVER['HTTP_X_FORWARDED_SSL'] == 'on'))) && (isset($this->data['settings_general']['seocms_url_secure']) && (isset($this->data['settings_general']['seocms_url_secure']) && $this->data['settings_general']['seocms_url_secure'] != 'http'))) {
		...
		public function router()
		{
				if (!$this->registry->get("request_blog_router")) {
                    ...
                    $this->registry->set('sc_router', 'record/' . $this->flag_router);
		...
		public function rewrite($link)
		{
			...
			if ($this->cache_rewrite_flag) {
				$cache_hach_link = (string)$this->config_scheme.'_'.(string)$this->config->get('config_language_id').'_'.(string)$this->config->get('config_store_id').'_'.(string)(md5($link));
				if (isset($this->cache_rewrite_data['links'][$cache_hach_link])) {
		            $link = $this->cache_rewrite_data['links'][$cache_hach_link];
		            if (!$is_blog_work) {
						$this->config->set("blog_work", false);
					}
	            	return $link;
			...
			if ($seocms_force_flag == false) {
				if (isset($url_data['path']) && $url_data['path']) {
					if ($url_data['path'] != $this->url_search) {
	                    ...
	                    if ($this->cache_rewrite_flag) {
		                    $this->cache_rewrite_data['links'][$cache_hach_link] = $link_original;
			...
			if ($this->config->get('config_seo_url') || (isset($this->data['settings_general']['tags_widget_status']) && $this->data['settings_general']['tags_widget_status'])) {
				...
				if ($url) {
                    ...
                    if ($this->cache_rewrite_flag) {
	                    $this->cache_rewrite_data['links'][$cache_hach_link] = $link;
	                    $this->cache->set($this->cache_rewrite_file, $this->cache_rewrite_data);
                    }
		            if (!$is_blog_work) {
						$this->config->set("blog_work", false);
		...
		private function getRecordbyTag($tag)
		{
			...
			if ($tag != '') {
				...
				AND
				rd.language_id = '" . (int)$this->config->get('config_language_id') . "'
				LIMIT 1";
