...
class agooCache extends Controller
{
    ...
    private $config_ssl_server = false;
	...
	public function __call($name, array $params)
	{
		...
		if ((isset($settings_general['seocms_url_secure']) && $settings_general['seocms_url_secure'] == 'https' && $settings_general['seocms_url_secure'] != 'http') || ((isset($_SERVER['HTTPS']) && ($_SERVER['HTTPS'] == 'on' || $_SERVER['HTTPS'] == '1')) || (!empty($_SERVER['HTTP_X_FORWARDED_PROTO']) && (strtolower($_SERVER['HTTP_X_FORWARDED_PROTO']) == 'https') || (!empty($_SERVER['HTTP_X_FORWARDED_SSL']) && $_SERVER['HTTP_X_FORWARDED_SSL'] == 'on'))) && (isset($settings_general['seocms_url_secure']) && $settings_general['seocms_url_secure'] != 'http')) {
			$this->config_ssl_server = true;
		}
	...
	public function set_agoo($key, $value)
	{
		...
		$file   = $this->dir_cache . 'cache.' . preg_replace('/[^A-Z0-9\._-]/i', '', $key).'.'.(int)$this->config_ssl_server.'.' . (time() + $this->expire);
		$handle = @fopen($file, 'w');
	...
	{
		$files = glob($this->dir_cache . 'cache.' . preg_replace('/[^A-Z0-9\._-]/i', '', $key).'.'.(int)$this->config_ssl_server.'.*');
		if ($files) {
			$exceptionizer = new PHP_Exceptionizer(E_ALL);
               /*
			    try {
